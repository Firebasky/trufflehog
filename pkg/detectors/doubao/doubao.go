...
var (
	...
	SKToken = regexp.MustCompile(`\b([a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})[\"';\s]*`)
)

// Keywords are used for efficiently pre-filtering chunks.
// Use identifiers in the secret preferably, or the provider name.
func (s Scanner) Keywords() []string {
	return []string{"api-key", "Ark", "api_key", "model="}
}

func (s Scanner) Description() string {
	return "zijie doubao sk token"
}
...
func (s Scanner) FromData(ctx context.Context, verify bool, data []byte) (results []detectors.Result, err error) {
	...
	for _, match := range matches {
		...
		s1 := detectors.Result{
			DetectorType: detector_typepb.DetectorType_Doubao,
			Raw:          []byte(tokenMatch),
		}
...
func verifyBaiLian(ctx context.Context, client *http.Client, tokenMatch string) (bool, map[string]string, error) {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, DoubaoURL, bytes.NewBuffer(jsonBody))
	if err != nil {
		return false, nil, err
	}
	...
	if strings.Contains(string(respBody), "The API key in the request is missing or invalid") {
		return false, nil, err
	}
...
func (s Scanner) Type() detector_typepb.DetectorType {
	return detector_typepb.DetectorType_Doubao
}
